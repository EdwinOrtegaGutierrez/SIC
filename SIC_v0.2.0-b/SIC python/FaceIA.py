...
class FaceDetection():
    ...
    def EntrenadorRF():
        ...
        face_recognizer.write(f'{myPath}/modeloEigenFace.xml')
        print("IA ENTRENADA")
    ...
    def Reconocimiento():        
        ...
        face_recognizer.read(f'{myPath}/modeloEigenFace.xml')
        ...
        while True:
            ...
            cv2.imshow('frame', frame)
            k = cv2.waitKey(1)
            if k == 27 or AA >= 25 or AD >= 25:
                break
            
        cap.release()
        cv2.destroyAllWindows()

        if AA > 20: return "AUTHORIZED ACCESS"
